...
export class Sprite implements IRenderable {
  ...
  public move(x: number, y: number): void {
    ...
    this.modelMatrix = mat4.translate(this.modelMatrix, mat4.create(), [
      x,
      y,
      0,
    ]);
  ...
  public init(gl: WebGL2RenderingContext, shader: Shader): void {
    ...
    gl.bufferData(
      gl.ARRAY_BUFFER,
      new Float32Array(CONSTS.shapes.Square.vertices),
      gl.STATIC_DRAW
    );
    ...
    gl.bufferData(
      ...
      new Uint16Array(CONSTS.shapes.Square.indices),
      gl.STATIC_DRAW
    );

    this.textureCoordBuffer = gl.createBuffer();
    if (!this.textureCoordBuffer)
      throw new Error("Texture coordinate buffer creation failed");
    gl.bindBuffer(gl.ARRAY_BUFFER, this.textureCoordBuffer);
    gl.bufferData(
      gl.ARRAY_BUFFER,
      new Float32Array(CONSTS.shapes.Square.texCords),
      gl.STATIC_DRAW
    );
    ...
    this.modelMatrix = mat4.fromTranslation(
      mat4.create(),
  ...
  public render(
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexBuffer);
    const positionLocation = this.shader.getAttribLocation(
      Shader.VOCABULARY.attribute.position
    );
    ...
    gl.bindBuffer(gl.ARRAY_BUFFER, this.textureCoordBuffer);
    const texCoordLocation = this.shader.getAttribLocation(
      Shader.VOCABULARY.attribute.texture
    );
    ...
    gl.drawElements(
      gl.TRIANGLES,
      CONSTS.shapes.Square.indices.length,
      gl.UNSIGNED_SHORT,
      0
    );
